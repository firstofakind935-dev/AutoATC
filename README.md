...
When a station is selected, its real runways and taxiways are drawn from
`monitor/public/groundlayouts.json`, generated by
`scripts/build-ground-layouts.js` directly from the actual Ground Chart
SVGs in the community
[ptfs-charts](https://github.com/Treelon/ptfs-charts) repo (the same
source `data/charts/*.json` comes from) - not invented or hand-traced.
For each airport it:

1. Pulls every text label's exact position out of the chart SVG (a real
   Inkscape vector drawing, resolving nested `<g transform>` chains), in
   the chart's own local coordinate space.
2. Resolves each runway designator to a real point by finding its
   heading-degree label (e.g. `106°`) and the nearest occurrence of the
   designator text next to it - spatial, not order-based, so it isn't
   fooled by an unrelated same-text label elsewhere on the chart (e.g. an
   apron stand numbered "10").
3. Calibrates rotation and scale from a genuinely reciprocal pair of
   ...
   runway's real known heading. Every other label's position is rotated
   into true-compass space through this same transform, which is what
   makes the on-screen orientation correct regardless of which way the
   original chart happened to be drawn (these charts are generally *not*
   drawn north-up).
4. Groups repeated occurrences of the same taxiway letter/number (e.g.
   every "E" or every "D1") and chains them nearest-neighbor into a
   polyline, then connects each one to the runway centerline - so a
   taxiway's real designator and its real position relative to the runway
   both come from the chart, not a guess.

The whole layout is then uniformly rescaled to a fixed on-screen size
(same spirit as the schematic fallback below) so a small strip's
taxiways aren't sub-pixel next to a mile-long runway - it's schematic in
scale, not in labeling or relative arrangement. Airports where this
resolution fails (no clean reciprocal pair found in the chart - e.g.
IBAR/TVO have no runway data at all, SHV's seaplane lane and IUFO's
imprecise heading pair don't yield one) fall back to the old schematic
treatment: a fixed-length line through the airport's reference point
along the runway's real heading, labeled with its designator but with no
taxiways drawn.
