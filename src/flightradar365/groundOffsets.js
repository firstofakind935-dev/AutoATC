...
module.exports = {
  ...
  // x/y also had to be recalculated, not just zoom - per main.js's
  // ground-view code, viewBox.width/height = bbox.width/height * zoom,
  // but viewBox.x/y are the raw stored x/y verbatim, NOT re-centered
  // automatically - the original x/y was some human's interactively
  // panned-to center point at the OLD (smaller) zoom, saved together as
  // one {zoom, x, y} snapshot (see the commented-out console.log in
  // main.js's ground-view pan handler - that's literally how these
  // values were captured originally). Changing zoom alone while leaving
  // that old top-left corner in place shoved the frame off to the side
  // once the viewBox grew ~6.9x, which is why Keflavik briefly showed
  // "in the wrong spot" after only the zoom fix. Fixed by solving for
  // the new x/y that keep that same original center point, just with a
  // viewBox sized for the new zoom: center = oldX + (bbox.width*oldZoom)/2
  // (same for y), then newX = center - (bbox.width*newZoom)/2 (same for y).
  // The other airports below still use IRFD's un-recalibrated scale and
  // may have the same relative-size issue - fix them the same way (zoom
  // AND x/y together) once real measurements are available for them too.
