...
function adaptPositionsToAircraftData(rows) {
    ...
    for (const row of rows) {
        ...
        const anchor = groundOffsetsMap.get(p.referenceAirport);
        if (!anchor || typeof p.distanceNm !== 'number' || typeof p.bearingDeg !== 'number') continue;

        const rad = (p.bearingDeg * Math.PI) / 180;
        const offsetSvgUnits = (p.distanceNm * STUDS_PER_NM) / 100;
        const x = (anchor.x + offsetSvgUnits * Math.sin(rad)) * 100;
        const y = (anchor.y - offsetSvgUnits * Math.cos(rad)) * 100;
