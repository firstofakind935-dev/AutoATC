...
// Everything in this block (and its call sites below, each marked
// "AutoATC adapter") is the only intentional deviation from the upstream
// 24radar.xyz source: swapping the live PTFS game telemetry this file was
// built against for AutoATC's own bot-estimated distance/bearing position
// feed. UI, layout and interaction code is untouched.
//
// SECOND deviation, added later: the world map itself. GroundOffsets.js's
// x/y (studs/100, "SVG units") is upstream 24radar.xyz's own coordinate
// system, tied to boundaries.svg/coast.svg's hand-traced vector island
// shapes - those never reliably agreed with GROUND.svg or with each other
// (see this repo's own commit history for how much churn that caused).
// worldMapAnchors.json is a completely separate, independently-verified
// coordinate system: pixel positions on public/assets/world-map.png (a
// single raster screenshot of the real PTFS world map), each tagged with
// how it was derived (see that file's own "note" field). The world map
// background and every world-map-space position below (aircraft anchors,
// the airport marker layer) use ONLY this new system - GroundOffsets.js
...
// text (e.g. "Belta-123"), so CallsignMap's carrier lookup doesn't apply -
// fall back to the raw callsign whenever it doesn't match that convention.
function displayCallsign(id) {
    const parts = id.split('-');
    const mapped = callsignMap.get(parts[0]);
    return mapped ? mapped + (parts[1] || '') : id;
}

// Likewise, AutoATC's aircraft type strings won't all be in 24radar's
// hand-curated map of common PTFS types - show the raw type rather than
// "undefined" when it's missing.
function displayAcftType(type) {
    return acftTypeMap.get(type) || type || 'A/C';
}

function displayAirline(id) {
    const parts = id.split('-');
    return airlineMap.get(parts[0]) || 'Unknown Operator';
}

// Converts one row from AutoATC's GET /api/dashboard/positions
// (`{ callsign, aircraftType, speed, position: { distanceNm, bearingDeg,
// referenceAirport, altitudeFt, headingDeg }, ageMs }`, see monitor/server.js
// and src/atc/positions.js) into the `{ [callsign]: {...} }` shape this file
// expects from the live 24radar.xyz backend (see sample.json). AutoATC has
// no flight-plan, wind, or on-ground telemetry, so those fields are left
// null/placeholder - flightPlan=null already makes the Departures/Arrivals
// tables and route-based label coloring no-op gracefully (they all check
// `if (info.flightPlan)` first), which is the intended degradation here
// rather than inventing data AutoATC doesn't have.
function adaptPositionsToAircraftData(rows) {
    const result = {};
    for (const row of rows) {
        const p = row.position || {};
        const anchor = worldMapAnchorsMap.get(p.referenceAirport);
        if (!anchor || typeof p.distanceNm !== 'number' || typeof p.bearingDeg !== 'number') continue;

        // *100 here (and /100 at every consumer below, e.g.
        // updateAssignedVectorLayer's x1/y1) matches the convention the
        // rest of this file already uses for aircraft position storage -
        // keeping it means only the anchor source and scale changed, not
        // every downstream consumer of info.position.
        const rad = (p.bearingDeg * Math.PI) / 180;
        const x = (anchor.x + p.distanceNm * WORLD_MAP_PX_PER_NM * Math.sin(rad)) * 100;
        const y = (anchor.y - p.distanceNm * WORLD_MAP_PX_PER_NM * Math.cos(rad)) * 100;

        result[row.callsign] = {
            heading: typeof p.headingDeg === 'number' ? p.headingDeg : 0,
            altitude: typeof p.altitudeFt === 'number' ? p.altitudeFt : 0,
            playerName: row.callsign,
            aircraftType: row.aircraftType || 'Unknown',
            position: { x, y },
...
function updateAssignedVectorLayer(data) {
    ...
    const lengthSvgUnits = ASSIGNED_VECTOR_LENGTH_NM * WORLD_MAP_PX_PER_NM;
...
function updateOverlay(id, info) {
    ...
    infoOverlay.querySelector('#callsign-bar').innerHTML = displayCallsign(id);
    infoOverlay.querySelector('#player-name').innerHTML = info.playerName;
    infoOverlay.querySelector('#aircraft-tag').innerHTML = displayAcftType(info.aircraftType);
    infoOverlay.querySelector("#detailed-info").innerHTML = `
                <span style="margin-bottom: 4px;">${displayAirline(id)}</span>
                <span>Aircraft Type : (${displayAcftType(info.aircraftType)})</span>
...
// map - which they never reliably did (see worldMapAnchorsMap's own
// comment near the top of this file). Rather than re-guess a transform
// for each airport's diagram, this draws a simple marker + label at that
// airport's verified worldMapAnchorsMap position instead - correct by
// construction, since it's the exact same anchor aircraft positions are
// computed from. The detailed GROUND.svg diagram itself is still shown,
// just in the dedicated Ground View side panel (loadGroundChartSVG(),
// unaffected by this change) rather than overlaid on the world map.
function loadAirportData(airportSelector) {
    ...
    const anchor = worldMapAnchorsMap.get(folder);
    const mapSvg = document.getElementById('boundaries-svg');
    if (!anchor || !mapSvg) return;

    const marker = document.createElementNS('http://www.w3.org/2000/svg', 'g');
    marker.setAttribute('id', 'groundview-svg');
    marker.style.display = groundViewVisible ? 'block' : 'none';

    const dot = document.createElementNS('http://www.w3.org/2000/svg', 'circle');
    dot.setAttribute('cx', anchor.x);
    dot.setAttribute('cy', anchor.y);
    dot.setAttribute('r', 3);
    dot.setAttribute('fill', '#927d16');
    dot.setAttribute('stroke', '#f5d67d');
    dot.setAttribute('stroke-width', 0.8);
    marker.appendChild(dot);

    const label = document.createElementNS('http://www.w3.org/2000/svg', 'text');
    label.setAttribute('x', anchor.x + 5);
    label.setAttribute('y', anchor.y - 4);
    label.setAttribute('fill', '#f5d67d');
...
function generateATIS() {
    ...
    // AutoATC adapter: aircraftData can legitimately be empty (no traffic
    // tracked yet), unlike the live PTFS game this was built against - fall
    // back instead of throwing on the missing entry.
    const windParts = Object.values(aircraftData)[0]?.wind?.split("/") || ["000", "00"];
    const dir = windParts[0].padStart(3, '0');
    const speed = windParts[1].padStart(2, '0');

    // AutoATC adapter: a handful of AutoATC's own airports have no official
    // ptfs-charts page (ChartsLinkPath is null for those, see AirportInfo.js).
    const chartsPath = airportInfo[airportSelector.value].ChartsLinkPath;
    const chartsLink = charts == 'Offical' && chartsPath ? `https://github.com/Treelon/ptfs-charts/tree/main/${chartsPath}` : '';
...
// AutoATC adapter: the upstream 24radar.xyz backend that used to live at
// api.24radar.xyz (REST poll below + a WebSocket that main.js never actually
// read from - it only logged open/close) is replaced by AutoATC's own
// same-origin position feed. See adaptPositionsToAircraftData() above.
...
async function loadApproachList(icao) {
    ...
    const res = await fetch(`/365radar-api/approaches/${icao}`)
...
async function atisLetter(icao) {
    try {
        const response = await fetch(`/365radar-api/atis/${icao}`);
        if (!response.ok) throw new Error("Network response was not ok");
...
function addDepartureRow(info, id) {
    ...
    const columns = [
        ...
        { name: 'type', type: 'text', value: displayAcftType(info.aircraftType) },
...
function updateGroundLabel(group, info, id) {
    ...
    text.innerHTML = `
        <tspan id='span1' dx="0" dy="0em">${star + displayCallsign(id) + ' ' + info.speed}kt ${displayAcftType(info.aircraftType)}</tspan>
    `;
    if (info.flightPlan == null) {
...
function updateLabel(group, info, id) {
    ...
    text.innerHTML = `
        <tspan dx="0" dy="0em" id="tspan1">${displayCallsign(id) + star}</tspan>
    ...
    text.innerHTML = `
        <tspan id="tspan1" dx="0" dy="0em">${displayCallsign(id) + star}</tspan>
        <tspan id="tspan2" dx="-${(tspan1 + 6 * currentZoom)}" dy="1.2em">${info.altitude}ftㅤ${info.speed}kt</tspan>
        <tspan id="tspan3" dx="-${(tspan2 + 6 * currentZoom)}" dy="1.2em">${info.heading}°ㅤㅤ${displayAcftType(info.aircraftType)}</tspan>
    `;
}
...
function fetchMapLayer(container) {
    ...
    Promise.resolve()
        .then(async () => {
            ...
            // AutoATC adapter: boundaries.svg (FIR/sector lines hand-traced
            // against the old coast.svg vector coordinate system) is
            // intentionally NOT loaded here anymore - it has no correct
            // position in the new world-map.png pixel space, and overlaying
            // it wrongly would reintroduce exactly the misalignment bugs
            // this whole change exists to get away from. #boundaries-svg
            // still needs to exist as an empty mount point, though:
            // loadAirportData() below appends each airport's marker into it,
            // and the "bounds-check" toggle in the toolbar still targets its
            // .style.display.
            Promise.resolve()
                .then(async () => {
                    const svg = document.getElementById('map-svg');
                    const boundaries = document.createElementNS('http://www.w3.org/2000/svg', 'g');
